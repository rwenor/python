...
def GetDiffDbImg(buffer1, buffer2):
    ...
    PTD('End')
    dprt("Change: %s changed pixel, maxDiff = %s, 5 = %s 10 = %s, 15 = %s" % \
...
#    q = multiprocessing.Queue()
    ...
    tw = multiprocessing.Process(target=img_worker, args=(queue, queue2, None,0, None))
    ##tw = threading.Thread(target=img_worker, args=(queue, queue2, None,0, None))
    tw.start()

    tw2 = multiprocessing.Process(target=img_worker, args=(queue, queue2, None,0, None))
    tw2.start()
    tw3 = multiprocessing.Process(target=img_worker, args=(queue, queue2, None,0, None))
    tw3.start()
